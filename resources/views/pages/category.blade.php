<x-guest-layout title="Entradas por categoria: {{ $category}}">
    @seo(['description' => "Contenido con la categoria {$category} en el blog."])
    ...
    <div
        class="max-w-screen-lg prose dark:prose-invert prose-h1:tracking-tight mx-auto px-2 sm:px-6 lg:px-8 my-4 sm:my-12 lg:my-16">
        ...
        <div class="max-w-screen-lg mx-auto px-2 sm:px-6 lg:px-8 space-y-12 py-16">
            ...
            <article class="relative flex flex-col max-w-3xl lg:ml-auto lg:max-w-none lg:w-[50rem]">
                <h3 class="text-xl tracking-tight font-bold dark:text-white dark:hover:text-orange-300">
                    <a class="underline dark:text-white dark:hover:text-orange-300" href="{{ $post->link() }}">{{
                        $post->title }}</a>
                </h3>
                ...
                <div
                    ...
                    <dl>
                        ...
                        <dd class="text-sm leading-7 lg:absolute lg:top-0 lg:right-full sm:mr-8 lg:whitespace-nowrap">
                            <a class="dark:hover:text-orange-300"
                                href="{{ route('page.blog.monthly', ['year' => $post->date->year, 'month' => $post->date->format('m')]) }}">{{
                                Str::ucfirst($post->date->monthName) }}</a> {{$post->date->year }}
                        </dd>
                    </dl>
                </div>
            </article>
