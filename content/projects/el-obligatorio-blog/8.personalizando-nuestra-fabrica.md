...
Si corremos `Pest` todo `PASA`, la propiedad de `date` se actualiza con los valores que pasamos como argumento y el método `createPostFile()` usa la fecha correcta al crear el archivo.
...
Para poder activar esta funcionalidad tendremos que hacer unos cambios en nuestra fabrica. Por ejemplo agregaremos una nueva propiedad llamada `hidden` y haremos los cambios necesarios en el método `createPostFile()` para usar esta nueva propiedad.
